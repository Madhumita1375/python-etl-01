...
# Get batch info from environment or master function
...
print(f"\nRunning customers_history ETL | Batch No: {ETL_BATCH_NO} | Date: {ETL_BATCH_DATE}\n")
...
try:
    ...
    update_sql = f"""
    UPDATE j25madhumita_devdw.customer_history AS hist
    SET 
    ...
    FROM j25madhumita_devdw.customers AS cust
    WHERE 
    ...
    insert_sql = f"""
    INSERT INTO j25madhumita_devdw.customer_history (
        dw_customer_id,
        creditLimit,
        effective_from_date,
    ...
    FROM j25madhumita_devdw.customers AS cust
    LEFT JOIN j25madhumita_devdw.customer_history AS hist
